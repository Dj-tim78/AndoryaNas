              ...
              <div className="flex items-start justify-between mb-6">
                ...
                <div className="flex flex-col items-end gap-2">
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => setPoolToRename(pool)} className="p-2 rounded-xl bg-zinc-950 text-zinc-500 hover:text-indigo-400 border border-zinc-800 transition-all">
                      <Edit3 size={16} />
                    </button>
                    <button onClick={() => setPoolToDelete(pool)} className="p-2 rounded-xl bg-zinc-950 text-zinc-500 hover:text-rose-500 border border-zinc-800 transition-all">
                      <Trash2 size={16} />
                    </button>
                  </div>
        ...
        <div className="flex items-center justify-between px-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">Lecture/Écriture et IOPS en temps réel.</p>
            </div>
          </div>
        </div>
            ...
            <div className="h-[280px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={displayData}>
                  ...
                  <ChartTooltip contentStyle={{ backgroundColor: '#09090b', borderColor: '#27272a', borderRadius: '12px', fontSize: '10px' }} />
          ...
          {availableDisks.map(disk => (
            <div key={disk.id} className="p-6 bg-zinc-950 border-2 border-dashed border-zinc-800 rounded-[2.5rem] hover:border-amber-500/40 transition-all flex flex-col group animate-in zoom-in-95">
                ...
                <div className="mt-6 flex gap-2">
                  <button onClick={() => startInitialization(disk)} className="flex-1 py-3 bg-amber-500 hover:bg-amber-400 text-zinc-950 rounded-2xl text-xs font-bold transition-all shadow-lg flex items-center justify-center gap-2">
      ...
      {diskToRemove && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-zinc-950/90 backdrop-blur-md">
          <div className="w-full max-w-sm bg-zinc-900 border border-rose-500/30 rounded-[2.5rem] p-10 shadow-3xl text-center space-y-6">
            <LogOut size={32} className="text-rose-500 mx-auto" />
            <h3 className="text-xl font-bold">Retrait Matériel</h3>
            <p className="text-sm text-zinc-500">Voulez-vous retirer {diskToRemove.model} ?</p>
            <div className="flex gap-4">
              <button onClick={() => setDiskToRemove(null)} className="flex-1 py-4 bg-zinc-800 rounded-2xl">Annuler</button>
              <button onClick={handleEjectDisk} className="flex-1 py-4 bg-rose-600 rounded-2xl">Confirmer</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="w-full max-w-2xl bg-zinc-900 border border-zinc-800 rounded-[3rem] p-10 shadow-3xl">
            {initStep === 'config' && (
              <div className="space-y-6">
                ...
                <input type="text" value={configName} onChange={e => setConfigName(e.target.value)} className="w-full px-6 py-4 bg-zinc-950 border border-zinc-800 rounded-2xl text-white outline-none" />
                <div className="flex gap-4">
                  <button onClick={() => setConfiguringDisk(null)} className="flex-1 py-4 bg-zinc-800 rounded-2xl text-white">Annuler</button>
            ...
            {initStep === 'progress' && (
              <div className="text-center py-10 space-y-6">
                ...
                <p className="text-zinc-500">Formatage en cours...</p>
              </div>
            )}
            {initStep === 'success' && (
              <div className="text-center py-10 space-y-6">
                <CheckCircle2 size={64} className="text-emerald-500 mx-auto" />
                <h3 className="text-2xl font-bold text-white">Volume prêt !</h3>
